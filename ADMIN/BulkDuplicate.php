<?php $PageTitle = 'BulkDataDuplicate';
...
<body>
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle">
            <h1>Bulk Data Duplicates</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="">Home</a></li>
                    <li class="breadcrumb-item active">Bulk Data Duplicates</li>
                </ol>
            </nav>
        </div>
        ...
        <section class="section dashboard">
            ...
            <div class="card card-body table-card p-0 px-3 py-1">
                ...
                <table class="table table-striped table-bordered display" id="DuplicateTable" style="width:100%;">
                    <thead class="text-center">
                        <tr>
                            ...
                            <th class="">Sl.No</th>
                            <th class="">Call</th>
                            <th class="">Imp Data</th>
                            <th class="">Data Id</th>
                            <th class="">Mobile</th>
                            <th class="">Ref.Mobile</th>
    ...
    <script>
        ...
        let PageLength = $('#PageLengthTable').val();

        /* Formatting function for row details - modify as you need */
        // function format(d) {
        //     // `d` is the original data object for the row
        //     return (
        //         '<table class="table table-bordered" cellpadding="" cellspacing="1" border="2">' +
        //         '<tr>' +
        //         '<td> </td>' +
        //         '<td> </td>' +
        //         '<td> </td>' +
        ...
        //     );
        // }
        var itemTable = $('#DuplicateTable').DataTable({
            "processing": true,
            "ajax": "DuplicateDatas.php?BulkDuplicate",
            //"responsive": true,
            // "scrollX": true,
            "fixedHeader": true,
            ...
            "initComplete": function() {
                //console.log("helloo");
                $("#DuplicateTable").wrap("<div style='overflow:auto; width:100%;height:70vh;position:relative;'></div>");
            },
            "columns": [
                // {
                ...
                },
                {
                    ...
                    data: null,
                    "render": function(data, type, row, meta) {
                        let NewData = '<div class="text-center"> <strong> ' + data.bulkPhone + ' </strong> </div>';
                        if ( (data.bulkDuplicate).trim() == '' && (data.referenceDuplicate).trim() == '' && (data.companyDuplicate).trim() == '') {
                            NewData += '<a class="text-danger"><strong> Record Deleted </strong> </a>';
                        } else {
                            if (type == 'display') {
                                let DuplicateVar = data.bulkDuplicate;
                                DuplicateArray = DuplicateVar.split("/");
                                for (let i = 0; i < DuplicateArray.length; i++) {
                                    //console.log(DuplicateArray[i]);
                                    if (NewData == '<div class="text-center"> <strong> ' + data.bulkPhone + ' </strong> </div>') {
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        // if (type == 'display') {
                        //     data = data.referPhone + '<br>' + data.referenceDuplicate
                        // }
                        // return data;

                        let NewData = '<div class="text-center"> <strong> ' + data.referPhone + ' </strong> </div>';
                        if (type == 'display') {
                            let DuplicateVar = data.referenceDuplicate;
                            DuplicateArray = DuplicateVar.split("/");
                            for (let i = 0; i < DuplicateArray.length; i++) {
                                //console.log(DuplicateArray[i]);
                                if (NewData == '<div class="text-center"> <strong> ' + data.referPhone + ' </strong> </div>') {
                                    NewData += '<a href="Lapview.php?FindMember=' + DuplicateArray[i] + '">' + DuplicateArray[i] + '</a>';
                                } else {
                                    NewData += ' /  <a href="Lapview.php?FindMember=' + DuplicateArray[i] + '">' + DuplicateArray[i] + '</a>';
                                }
                    ...
                    "render": function(data, type, row, meta) {
                        if (type == 'display') {
                            data =
                                ...
                                '<button class="ConvertBtn border-1 btn shadow-none py-0 px-2" type="button" data-convert="' + data.importedData + '" value="' + data.bulkId + '" >Convert</button>';
                        }
                        return data;
                    }
                },
        ...
        // $('#DuplicateTable tbody').on('click', 'td.dt-control', function() {
        ...
        $('#PageLengthTable').on('change', function() {
            var PageLength = $(this).val();
            itemTable.page.len(PageLength).draw();
        });

        //delete duplicate
        $('#DuplicateTable tbody').on('click', '.DeleteBtn', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (TestJson(data) == true) {
                                ...
                                } else if (delResponse.DeleteBulkDuplicate == 2) {
                                    $('#ResponseImage').html(
        ...
        //convert to bulk reg
        $('#DuplicateTable tbody').on('click', '.ConvertBtn', function(e) {
            e.preventDefault();
            ...
            $('#convertYes').click(function() {
                if (ConvertValue != null) {
                    $.ajax({
                        ...
                        url: "DuplicateOperation.php",
                        data: {
                            BulkConvertId: ConvertValue,
                            BulkConvertType: ConvertType
                        },
                        beforeSend: function() {
                            $('#loading').show();
                        ...
                        success: function(data) {
                            ...
                            if (TestJson(data) == true) {
                                ...
                                if (ConvertResponse.BulkConvertDuplicate == 0) {
                                    ...
                                    $('#confirmModal').modal('show');
                                    itemTable.ajax.reload();
                                } else if (ConvertResponse.BulkConvertDuplicate == 2) {
        ...
        $(document).on('click', '.DeleteAll', function() {
            ...
            if (confirm("Do you want to delete all duplicate data?") == true) {
                $.ajax({
                    type: "POST",
                    url: "DuplicateOperation.php",
                    data: {
                        BulkDeleteAllID: DeleteAllID
                    },
                    beforeSend: function() {
                        $('#loading').show();
                        $('.mainContents').hide();
                    ...
                    success: function(data) {
                        ...
                        if (TestJson(data) == true) {
                            ...
                            if (delAllResponse.BulkDeleteAllDuplicate == 0) {
                                $('#ResponseImage').html(
                                    '<img src="./warning.jpg" style="height:130px;width:130px;" class="img-fluid" alt="">'
                                );
                                $('#ResponseText').text("Branch is Already in Use");
                                $('#confirmModal').modal('show');
                            } else if (delAllResponse.BulkDeleteAllDuplicate == 1) {
                                $('#ResponseImage').html(
                                    '<img src="./success.jpg" style="height:130px;width:130px;" class="img-fluid" alt="">'
                                );
                                $('#ResponseText').text(
                                ...
                                $('#confirmModal').modal('show');
                                itemTable.ajax.reload();
                            } else if (delAllResponse.BulkDeleteAllDuplicate == 2) {
                                $('#ResponseImage').html(
                                    '<img src="./error.jpg" style="height:130px;width:130px;" class="img-fluid" alt="">'
                                );
        ...
        $(document).on('click', '.ConvertAll', function() {
            ...
            if (confirm("Do you want to convert all duplicate data?") == true) {
                $.ajax({
                    type: "POST",
                    url: "DuplicateOperation.php",
                    data: {
                        BulkConvertAllID: ConvertAllID
                    },
                    beforeSend: function() {
                        $('#loading').show();
                        $('.mainContents').hide();
                    ...
                    success: function(data) {
                        ...
                        if (TestJson(data) == true) {
                            ...
                            if (ConvAllResponse.BulkConvertAllDuplicate == 0) {
                                $('#ResponseImage').html(
                                    '<img src="./warning.jpg" style="height:130px;width:130px;" class="img-fluid" alt="">'
                                );
                                $('#ResponseText').text("Branch is Already in Use");
                                $('#confirmModal').modal('show');
                            } else if (ConvAllResponse.BulkConvertAllDuplicate == 1) {
                                $('#ResponseImage').html(
                                    '<img src="./success.jpg" style="height:130px;width:130px;" class="img-fluid" alt="">'
                                );
                                $('#ResponseText').text(
                                ...
                                $('#confirmModal').modal('show');
                                itemTable.ajax.reload();
                            } else if (ConvAllResponse.BulkConvertAllDuplicate == 2) {
                                $('#ResponseImage').html(
                                    '<img src="./error.jpg" style="height:130px;width:130px;" class="img-fluid" alt="">'
                                );
