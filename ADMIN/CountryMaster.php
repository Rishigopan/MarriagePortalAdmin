            ...
            <div class="modal-content cntrymodalbg">
                ...
                <div class="modal-body">
                    <form action="" id="AddMaster" class="AddForm">
                        <div class="row pt-3">
                            <div class="col-12">
                                <label for="country_name" class="mt-3">Country</label>
                                <input class="form-control mb-3" type="text" placeholder=" " id="country_name" name="CountryName" required>
                    ...
                    <form action="" id="UpdateMaster" class="UpdateForm" style="display:none;">
                        <div class="row pt-3">
                            <div class="col-12">
                                <input type="hidden" id="update_country_id" name="UpdateCountryId">
                                <label for="update_country_name" class="mt-3">Country</label>
                                <input class="form-control mb-3" type="text" id="update_country_name" placeholder=" " name="UpdateCountryName" required>
                                <div class="d-flex justify-content-center">
            ...
            <div class="container-fluid mainContents">
                <div class="card card-body main_card shadow-lg p-3 mb-5">
                    <div class="main_heading d-flex justify-content-between mb-2 shadow p-2">
                        ...
                        <div class="">
                            <button class="btn AddBtn mt-1" data-bs-toggle="modal" data-bs-target="#MasterModal">Add Country</button>
                        </div>
                    </div>
    ...
    <script>
        ...
        $(function() {
            ...
            validator.validator.custom = function(el, event) {
                if ($(el).is('#country_name') && $(el).val().trim().length == 0) {
        ...
        $('#MasterTable tbody').on('click', '.btn_delete', function() {
            ...
            $('#confirmYes').click(function() {
                if (delValue != null) {
                    $.ajax({
                        url: ApiLink + "/api/country/delete/" + delValue,
                        method: "DELETE",
                        async: true,
                        crossDomain: true,
                        headers: {
        ...
        $(function() {
            ...
            validator.validator.custom = function(el, event) {
                if ($(el).is('#update_country_name') && $(el).val().trim().length == 0) {
                    return 'Cannot be empty';
                }
                // else if($(el).is('#update_country_name') && $(el).val().match(/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?0-9]+/)){
                //     return 'Only allowed alphabets';
            ...
            $(document).on('submit', '#UpdateMaster', (function(e) {
                e.preventDefault();
                var UpdateId = $('#update_country_id').val();
                console.log(UpdateId);
                $.ajax({
                    url: ApiLink + "/api/country/update/" + UpdateId,
                    method: "PUT",
                    async: true,
                    crossDomain: true,
                    headers: {
                    ...
                    data: {
                        "countryName": $('#update_country_name').val(),
                        "updatedBy": EmpID,
        ...
        $('#MasterTable').on('click', '.btn_edit', function() {
            ...
            console.log(EditId);
            $.ajax({
                url: ApiLink + "/api/country/viewid/" + EditId,
                method: "GET",
                async: true,
                crossDomain: true,
                headers: {
                ...
                $('#update_country_name').val(ViewEditValues.country);
                $('#update_country_id').val(EditId);
